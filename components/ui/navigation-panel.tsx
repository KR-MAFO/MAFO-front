...
import { ArrowUp, ArrowLeft, ArrowRight, Undo2 } from "lucide-react"
...
const DirectionIcon = ({
  ...
  if (instruction) {
    // 좌회전 관련 키워드
    if (
      instruction.includes("좌회전") ||
      instruction.includes("왼쪽") ||
      instruction.includes("좌측") ||
      instruction.includes("왼편") ||
      instruction.includes("왼편으로")
    ) {
    ...
    // 우회전 관련 키워드
    if (
      instruction.includes("우회전") ||
      instruction.includes("오른쪽") ||
      instruction.includes("우측") ||
      instruction.includes("오른편") ||
      instruction.includes("오른편으로")
    ) {
    ...
    // 유턴 관련 키워드
    if (
      instruction.includes("유턴") ||
      instruction.includes("돌아서") ||
      instruction.includes("되돌아")
    ) {
      return <Undo2 className="w-6 h-6 text-white" />
    }

    // 출발 관련 키워드
    if (
      instruction.includes("출발") ||
      instruction.includes("시작") ||
      instruction.includes("출발지")
    ) {
    ...
    if (
      ...
      instruction.includes("종료") ||
      instruction.includes("끝")
    ) {
    ...
    // 직진 관련 키워드
    if (
      instruction.includes("직진") ||
      instruction.includes("계속") ||
      instruction.includes("앞으로") ||
      instruction.includes("쭉")
    ) {
      return <ArrowUp className="w-6 h-6 text-white" />
    }
  }
  // direction 값에 따른 fallback
  switch (direction) {
    case "left":
      return <ArrowLeft className="w-6 h-6 text-white" />
    case "right":
    ...
    <div className="absolute top-4 left-4 right-4 max-h-[90vh] overflow-hidden bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg shadow-lg p-4 z-20 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-white bg-opacity-20 rounded-full flex items-center justify-center text-2xl">
            <DirectionIcon direction={currentStep?.direction} instruction={currentStep?.instruction} />
          </div>
          <div>
      ...
      <div className="bg-white bg-opacity-10 rounded-lg p-3 overflow-y-auto max-h-40 mt-2 space-y-1 text-sm">
        {navigationInfo.steps.map((step, index) => (
          ...
          >
            <DirectionIcon direction={step.direction} instruction={step.instruction} />
